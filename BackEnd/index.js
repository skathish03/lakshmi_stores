...
mongoose.connect("mongodb+srv://athishsk21cse:cxrpDhSWY8JXFRgC@cluster0.cnhbqic.mongodb.net/");
//API CREATION
...
app.get('/popularinfurniture',async(req,res)=>{
    let products = await Product.find({category:"furniture"});
    let popular_in_furniture = products.slice(0,4);
    console.log("Popular in furniture fetched");
    res.setDefaultEncoding((popular_in_furniture));
})
...
app.post('/details', fetchUser, async (req, res) => {
    try {
        console.log("getting....",req.user.id);
